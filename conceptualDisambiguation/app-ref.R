# # Read in files 
# # Assume filepath is "input.json"

# # If no "input.json", show console/warning for specifying another file. 
# # Output nice error message if no input.json

# # Construct Conceptual Disambiguation interface
# ui <- fluidPage(
#   # ======== Basic layout ========
#     shinyjs::useShinyjs(),
...
#   # ======== Modules ========
#   # conceptualModuleUI and cycleUI are defined in R/construct_modules.R
#   conceptualModelUI(),
#   cycleUI()
# )
...
#   # ======== Modules ========
#   # All module servers are defined in R/example-module.R
#   conceptualModelServer(input, output), 
#   conceptualModelQuestionsServer(input, output), 
#   returnValuesServer(input, output)
#   # =========================
# }

# START HERE: Start with one UI module at a time --> Add code, add server test (FIRST: Set up test files): https://shiny.rstudio.com/articles/testing-overview.html
#   # **GOAL: add and pass tests for conceptual model disambiguation**
generateConceptualModelQuestions <- function(id) {
  uiOutput(id)
}

generateConceptualModelGraph <- function(id) {
  p("This is what your conceptual model looks like:")
  plotOutput(id)
}

# TODO: Add module Server


conceptualDisambiguationApp <- function(conceptualModel, iv, dv, inputFilePath) {
  # print("in conceptual app")
  # print(inputFilePath)
      ...
      column(8, offset=2, 
        tabsetPanel(
          ...
          tabPanel("Conceptual model",
                  ...
                  generateConceptualModelQuestions("cmQuestions"),
                  generateConceptualModelGraph("cmGraph"),
                  actionButton("submit", "Update!") # Submit button
          )
        )
      )
    )
