...
def createRandomEffectsAddedCallbacks(app, comp: GUIComponents = None):
    ...
    inputs = [Input(id, "value") for id in comp.getRandomEffectCheckboxIds()]
        ...
        if args:
            ...
            for i in range(len(inputs)):
                ...
                if input and comp.hasRandomEffectForComponentId(id):
                    ...
                    value.append(randomEffect)
                    pass
                pass
            return [html.Li(str(c)) for c in value]
...
def createRandomEffectsCorrelationCallbacks(app, comp: GUIComponents = None):
    ...
    if comp:
        ...
        if checkboxIds:
            inputs = [Input(id, "value") for id in checkboxIds]
            ...
            def changeCorrelation(*args):
                ...
                if args:
                    ...
                    states = args[half:]
                    # dict of id to value
                    idToValue = {states[i]: inputs[i] for i in range(half)}
                    idToCorrelated = {}
                    for id, value in idToValue.items():
                        idToCorrelated[id] = (
                            " (correlated)" if value else " (not correlated)"
                        )
                        comp.markValueForCorrelatedId(id, value)
