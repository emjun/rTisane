...
cm<-assume(cm, cel) # strong belief that a grade 12 participant will have low income

ceh<-causes(cause=edu, effect=income, when=equals(edu, '5+ years of college'), then=increases(income))
cm<-hypothesize(cm, ceh) # participant with 5+ years of experience after college should have higher income 
```
...
script <- query(conceptualModel=cm, iv=age, dv=income)
```
