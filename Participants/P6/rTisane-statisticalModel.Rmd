...
cm <- ConceptualModel()
# cm <- assume/hypothesize(cm, causes(ses, testScore))
cm <- hypothesize(cm, causes(cause=race, effect=income))
cm <- hypothesize(cm, causes(cause=sex, effect=income))
cm <- hypothesize(cm, relates(lhs=sex, rhs=income))
cm <- hypothesize(cm, relates(lhs=race, rhs=income, when=equals(sex, 'Female'), then=increases(income)))
...
script <- query(conceptualModel=cm, iv=race, dv=income)
```
...
source("/Users/cse-loaner/Git/rTisane/Participants/P6/model.R") # You can copy and paste the script path that rTisane gives you, which should be something like "model.R"
